...
const fetchWebhooks = async () => {
  ...
  try {
    ...
    toast.value?.show(t('webhook.addError'), 'error');
  } finally {
    loading.value = false;
  }
};
...
const handleSave = async (webhookData) => {
  try {
    ...
    toast.value?.show(t('webhook.addSuccess'), 'success');
    closeModal();
    await fetchWebhooks();
  } catch (error) {
    console.error('Failed to save webhook:', error);
    toast.value?.show(error.data?.message || t('webhook.addError'), 'error');
  }
};
...
const testWebhook = async (id) => {
  ...
  try {
    ...
    if (response.success) {
      toast.value?.show(t('webhook.testSuccess'), 'success');
    } else {
      toast.value?.show(t('webhook.testFailed'), 'error');
    }
  } catch (error) {
...
const confirmDelete = async () => {
  ...
  try {
    ...
    toast.value?.show(t('webhook.deleteSuccess'), 'success');
    await fetchWebhooks();
  } catch (error) {
    console.error('Failed to delete webhook:', error);
    toast.value?.show(t('webhook.deleteError'), 'error');
  } finally {
