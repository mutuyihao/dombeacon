        ...
        <div class="flex items-start justify-between">
          <div class="flex-1">
            ...
            <div class="space-y-2 text-sm">
              <div class="flex items-center gap-2 text-text-secondary">
                ...
                <span class="font-mono text-xs">{{ maskSendKey(config.sendKey) }}</span>
              </div>
          ...
          <div class="flex items-center gap-2 ml-4">
            ...
            <button
              ...
              class="p-2 text-text-secondary hover:text-red-600 hover:bg-red-50 rounded transition-colors"
...
const fetchServerchan = async () => {
  ...
  try {
    ...
    toast.value?.show(t('serverchan.addError'), 'error');
  } finally {
    loading.value = false;
  }
};
...
const handleSave = async (configData) => {
  try {
    ...
    toast.value?.show(t('serverchan.addSuccess'), 'success');
    closeModal();
    await fetchServerchan();
  } catch (error) {
    console.error('Failed to save Server酱:', error);
    toast.value?.show(error.data?.message || t('serverchan.addError'), 'error');
  }
};
...
const testServerchan = async (id) => {
  ...
  try {
    ...
    if (response.success) {
      toast.value?.show(t('serverchan.testSuccess'), 'success');
    } else {
      toast.value?.show(t('serverchan.testFailed'), 'error');
    }
  } catch (error) {
...
const confirmDelete = async () => {
  ...
  try {
    ...
    toast.value?.show(t('serverchan.deleteSuccess'), 'success');
    await fetchServerchan();
  } catch (error) {
    console.error('Failed to delete Server酱:', error);
    toast.value?.show(t('serverchan.deleteError'), 'error');
  } finally {
