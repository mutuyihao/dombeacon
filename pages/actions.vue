<template>
  <div class="flex min-h-full flex-col gap-6 md:h-full md:min-h-0 md:overflow-hidden">
    ...
    <header class="shrink-0">
      <p class="eyebrow mb-2">{{ $t('action.queueEyebrow') }}</p>
      <div class="flex flex-wrap items-end justify-between gap-4">
        ...
        <div class="flex flex-wrap items-center gap-x-6 gap-y-3">
          <div class="flex flex-wrap items-center gap-2">
            <button type="button" class="btn-ghost px-3 py-1.5 text-xs disabled:opacity-50" :disabled="loading || archiveWorking" @click="toggleArchivedView">
              {{ showArchived ? $t('action.viewActive') : $t('action.viewArchived') }}
            </button>
            <button v-if="canArchiveActions" type="button" class="btn-ghost px-3 py-1.5 text-xs disabled:opacity-50" :disabled="archiveWorking" @click="archiveCurrentActions">
              {{ $t('action.archiveRecords') }}
            </button>
          </div>
    ...
    <div v-else class="grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-3">
      ...
      >
        ...
        <header class="flex items-start justify-between gap-4">
          <div class="min-w-0 flex-1">
            <NuxtLink
              ...
              class="font-mono block truncate text-[15.5px] font-bold tracking-tight text-text-main transition-colors hover:text-accent"
            >
        ...
        <dl class="mt-1 space-y-0">
          ...
          <div v-if="action.archivedAt" class="flex items-center justify-between py-2.5 text-xs">
            <dt class="text-text-tertiary uppercase tracking-[0.14em]">{{ $t('action.archivedAt') }}</dt>
            <dd class="font-mono text-text-main">{{ formatDate(action.archivedAt) }}</dd>
          </div>
        </dl>

        <div v-if="!showArchived" class="mt-5 flex items-center gap-2">
          <button
...
const archiveWorking = ref(false);
...
const fetchActions = async () => {
  ...
  try {
    ...
    toast.error(error?.message || 'Failed to fetch actions');
  } finally {
    loading.value = false;
  }
};
...
const canArchiveActions = computed(() =>
  !showArchived.value && total.value > 0,
);
const emptyMessage = computed(() =>
  showArchived.value ? t('action.noArchivedActions') : t('action.noActions'),
);

watch([statusFilter, priorityFilter, limit, showArchived], () => {
...
const toggleArchivedView = () => {
  ...
  if (nextArchived && statusFilter.value === 'OPEN') {
    statusFilter.value = '';
  } else if (!nextArchived && statusFilter.value === '') {
    statusFilter.value = 'OPEN';
  }
};

const archiveCurrentActions = async () => {
  if (!window.confirm(t('action.archiveRecordsConfirm'))) return;
  archiveWorking.value = true;
  try {
    const resp = await $fetch('/api/actions/archive', {
      method: 'POST',
      body: {
        status: statusFilter.value || undefined,
        priority: priorityFilter.value || undefined,
    ...
    if (resp?.code !== 0) throw new Error(resp?.msg || 'Failed to archive actions');
    toast.success(t('action.archiveSuccess'));
    await fetchActions();
  } catch (error) {
    toast.error(error?.message || t('action.archiveError'));
  } finally {
    archiveWorking.value = false;
  }
};
