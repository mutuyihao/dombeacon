      ...
      <section>
        <div class="grid grid-cols-2 divide-y divide-hairline lg:grid-cols-4 lg:divide-y-0">
          ...
          >
            ...
            <p class="font-display mt-5 text-[2.5rem] font-medium leading-none tracking-[-0.045em] text-text-main" data-numeric>
              {{ stat.value }}
            </p>
...
const fetchSummary = async () => {
  ...
  try {
    ...
    toast.error(t('costs.fetchError'));
  } finally {
    loading.value = false;
  }
};
...
const handleSave = async (costData) => {
  try {
    await $fetch('/api/costs', { method: 'POST', body: costData });
    toast.success(t('costs.addSuccess'));
    closeModal();
    await fetchSummary();
  } catch (error) {
    console.error('Failed to save cost:', error);
    toast.error(error.data?.message || t('costs.addError'));
  }
};
...
const confirmDelete = async () => {
  ...
  try {
    ...
    toast.error(t('costs.deleteError'));
  } finally {
    deleteDialog.value.isOpen = false;
