...
const exportDomains = async () => {
  ...
  try {
    ...
    toast.error(t('import.export.error'));
  } finally {
  ...
  try {
    ...
    if (response.success) {
      ...
      if (response.data.success > 0) {
        toast.success(t('import.import.successMessage', { count: response.data.success }));
      }
      if (response.data.failed > 0) {
        toast.warning(t('import.import.failedMessage', { count: response.data.failed }));
      }
      if (response.data.failed === 0) {
    ...
    toast.error(error?.data?.message || t('import.import.error'));
  } finally {
