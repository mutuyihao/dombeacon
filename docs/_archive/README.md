# Archived Docs

Historical material kept for context. None of these are the canonical
reference any more — start from [../README.md](../README.md) for current
docs.

## Per-feature implementation narratives

These were written as "what I just shipped" walkthroughs while the
features were being built. The canonical content has been folded into
[../features.md](../features.md). Originals preserved here for the
verbose code references and decision history.

- [i18n-implementation.md](i18n-implementation.md)
- [serverchan-implementation.md](serverchan-implementation.md)
- [ssl-implementation.md](ssl-implementation.md)
- [cost-tracking.md](cost-tracking.md)
- [csv-import-export.md](csv-import-export.md)
- [webhook-ui-implementation.md](webhook-ui-implementation.md)
- [ui-ux-improvements.md](ui-ux-improvements.md) — Phase 1 UI polish narrative
- [ui-ux-testing-checklist.md](ui-ux-testing-checklist.md) — manual QA pass for Phase 1
- [domain-detail-optimization.md](domain-detail-optimization.md) — domain detail page polish
- [v1.2-changelog.md](v1.2-changelog.md) — v1.2 shipped-feature changelog
- [ui-function-quality-improvement-plan.md](ui-function-quality-improvement-plan.md) — UI and interaction quality plan

## Planning drafts

- [v1-2-pwa-goofy-lynx.md](v1-2-pwa-goofy-lynx.md) — v1.2 implementation plan (now shipped; see [v1.2-changelog.md](v1.2-changelog.md) for what landed)
- [product-direction-plan.md](product-direction-plan.md) — early product direction notes
- [product-blueprint.md](product-blueprint.md) — full product blueprint
- [blueprint-implementation-status.md](blueprint-implementation-status.md) — blueprint vs implementation tracking
- [v1-implementation-summary.md](v1-implementation-summary.md) — v1.0 summary
- [v1-verification-checklist.md](v1-verification-checklist.md) — v1.0 verification pass
- [progress-report.md](progress-report.md) — v1.1-era progress snapshot
